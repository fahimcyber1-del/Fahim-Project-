...
import { JobDescriptionDashboard } from './JobDescriptionDashboard';
import { JobDescriptionList } from './JobDescriptionList';
import { JobDescriptionForm } from './JobDescriptionForm';
import { JobDescriptionDetail } from './JobDescriptionDetail';
import { Briefcase, List, PlusCircle, LayoutDashboard } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between mb-6 flex-shrink-0">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Job Descriptions</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Manage roles, responsibilities, and requirements</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 border-b border-slate-200 pb-4 flex-shrink-0 overflow-x-auto">
        ...
        >
          <List className="w-4 h-4" /> All JDs
        </button>
        <button
        ...
        >
          <PlusCircle className="w-4 h-4" /> New JD
        </button>
      </div>

      <div className="flex-1 overflow-hidden flex flex-col min-h-0">
        {viewState.type === 'dashboard' && <JobDescriptionDashboard records={records} />}
        {viewState.type === 'list' && (
          <JobDescriptionList 
            records={records}
