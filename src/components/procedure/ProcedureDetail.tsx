...
import { ArrowLeft, Edit3, GitCommit, User, Calendar, Tag, Paperclip, Download, History, Target, Users, AlertCircle } from 'lucide-react';
...
export function ProcedureDetail({ record, onBack, onEdit }: ProcedureDetailProps) {
    ...
    <div className="max-w-5xl mx-auto w-full pb-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={onEdit} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-sm font-bold text-sm transition-colors">
          <Edit3 className="w-4 h-4" /> Edit Procedure
        </button>
      </div>
         ...
         <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               ...
               <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
                   ...
                   <p className="text-sm text-slate-700 leading-relaxed">{record.scope}</p>
               </div>
            </div>
            ...
            {record.prerequisites && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl shadow-sm p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="font-bold text-amber-800 uppercase tracking-widest text-sm">Prerequisites</h3>
                  </div>
                  <p className="text-sm text-amber-900 leading-relaxed">{record.prerequisites}</p>
              </div>
            )}
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
               ...
               <div className="p-4 sm:p-6">
                  {record.steps ? (
                    ...
                    <p className="text-sm text-slate-500 italic text-center py-4">No procedure steps provided.</p>
                  )}

                  {record.subProcessSteps && record.subProcessSteps.length > 0 && (
                    <div className="mt-8 pt-6 border-t border-slate-100">
                      <h4 className="font-bold text-slate-800 uppercase tracking-widest text-xs mb-4">Detailed Sub-Process Steps</h4>
                      <div className="space-y-4">
                        {record.subProcessSteps.map((step) => (
                          <div key={step.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex gap-4">
                            ...
                            <div className="flex-1 space-y-2">
                              <p className="text-sm text-slate-900 leading-relaxed max-w-none whitespace-pre-wrap">{step.description}</p>
                              {step.expectedOutcome && (
                                <div className="flex items-start gap-2 text-xs text-emerald-700 bg-emerald-50 px-3 py-2 rounded">
                                  <Target className="w-4 h-4 flex-shrink-0 mt-0.5" />
                                  <span className="font-semibold">Outcome:</span>
                                  <span className="whitespace-pre-wrap">{step.expectedOutcome}</span>
                                </div>
                              )}
                            </div>
                          </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
               ...
               <div className="space-y-4">
                 <div className="flex items-start gap-3">
                   ...
                   <div>
                     <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Department & Type</p>
