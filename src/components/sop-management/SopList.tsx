...
export function SopList({ records, onView, onEdit, onDelete, onNew }: SopListProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden min-h-0">
      <div className="p-3 border-b border-slate-200 bg-slate-50 flex flex-row items-center justify-between gap-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <PlusCircle className="w-4 h-4" /> <span className="hidden sm:inline">New SOP</span>
          </button>
        </div>
      </div>
            ...
            {filteredRecords.map((record) => (
              <div key={record.id} className="group bg-white border border-slate-200 hover:border-indigo-300 rounded-xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col">
                ...
                <div className="mt-auto pt-4 border-t border-slate-100 grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider font-bold text-slate-500 mb-1">Status & Dept</p>
