...
import { Search, Filter, Plus, FileText, Download, FileSpreadsheet, Eye, Edit, Trash2, MoreHorizontal, LayoutGrid, List } from 'lucide-react';
...
export function IncomingQCList({ records, onNavigate, onView, onEdit, onDelete }: Props) {
  ...
  const styles = Array.from(new Set(records.map(r => r.style).filter(Boolean))) as string[];
    ...
    <div className="h-full flex flex-col bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
      <div className="p-4 lg:p-6 border-b border-slate-200 bg-slate-50 flex flex-col gap-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-xl font-black text-slate-900 tracking-tight">Incoming QC Records</h2>
            <p className="text-slate-500 text-sm font-medium">Manage inspections for raw materials</p>
          </div>
        ...
        {showFilters && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-white border border-slate-200 rounded-lg mt-2">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Style</label>
              <select 
                value={filterStyle} 
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Show</label>
              <select 
                value={itemsPerPage} 
      ...
      <div className="flex-1 overflow-auto bg-slate-50/50 min-h-[400px] pb-48">
        {viewMode === 'list' ? (
            ...
            <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200">
              <tr>
                <th className="px-4 py-3 font-bold text-slate-600 uppercase tracking-wider w-10">
                  <input type="checkbox" checked={selectedIds.size === paginated.length && paginated.length > 0} onChange={toggleSelectAll} className="rounded border-slate-300" />
                </th>
                ...
                <th className="px-4 py-3 font-bold text-slate-600 uppercase tracking-wider">Category</th>
                <th className="px-4 py-3 font-bold text-slate-600 uppercase tracking-wider">Supplier</th>
                <th className="px-4 py-3 font-bold text-slate-600 uppercase tracking-wider">Status</th>
                <th className="px-4 py-3 text-right font-bold text-slate-600 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 bg-white cursor-default">
              {paginated.length === 0 ? (
                ...
                paginated.map((record, index) => {
                  const isLastFew = index >= paginated.length - 2 && paginated.length > 3;
                  return (
                    <tr key={record.id} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-4 py-3">
                        <input type="checkbox" checked={selectedIds.has(record.id)} onChange={() => toggleSelect(record.id)} className="rounded border-slate-300" />
                      </td>
                      <td className="px-4 py-3 font-bold text-slate-900">{record.id}</td>
                      <td className="px-4 py-3 text-slate-600">{record.date}</td>
                      <td className="px-4 py-3">
                        <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-xs font-bold">{record.category}</span>
                      </td>
                      <td className="px-4 py-3 text-slate-900 font-medium">{record.supplier}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                          record.status === 'Passed' ? 'bg-emerald-100 text-emerald-700' :
                          record.status === 'Failed' ? 'bg-rose-100 text-rose-700' :
                          record.status === 'Partial Pass' ? 'bg-amber-100 text-amber-700' :
                          record.status === 'On Hold' ? 'bg-blue-100 text-blue-700' :
                        ...
                        <div className="flex items-center justify-end gap-2" ref={openActionMenuId === record.id ? menuRef : null}>
                          ...
                          <div className="relative">
                            ...
                            {openActionMenuId === record.id && (
                              <div className={`absolute right-0 w-48 bg-white border border-slate-200 rounded-md shadow-lg z-50 flex flex-col p-1 text-sm text-left ${isLastFew ? 'bottom-full mb-1' : 'top-full mt-1'}`}>
                                <button onClick={() => { onView(record); setOpenActionMenuId(null); }} className="px-3 py-2 text-slate-700 hover:bg-slate-50 flex items-center gap-2 rounded">
                                    <Eye className="w-4 h-4" /> View Details
                                </button>
                                <button onClick={() => { onEdit(record); setOpenActionMenuId(null); }} className="px-3 py-2 text-slate-700 hover:bg-slate-50 flex items-center gap-2 rounded">
                                    <Edit className="w-4 h-4" /> Edit Record
                                </button>
                                <div className="h-px bg-slate-100 my-1"></div>
                                <button onClick={() => { setOpenActionMenuId(null); }} className="px-3 py-2 text-slate-700 hover:bg-slate-50 flex items-center gap-2 rounded">
                                ...
                                <div className="h-px bg-slate-100 my-1"></div>
                                <button onClick={() => { if(confirm('Are you sure?')) onDelete(record.id); setOpenActionMenuId(null); }} className="px-3 py-2 text-rose-600 hover:bg-rose-50 flex items-center gap-2 rounded">
          ...
          <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {paginated.length === 0 ? (
              <div className="col-span-full py-8 text-center text-slate-500">No records found.</div>
            ) : (
              paginated.map(record => (
                <div key={record.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow relative group">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                       <input type="checkbox" checked={selectedIds.has(record.id)} onChange={() => toggleSelect(record.id)} className="rounded border-slate-300 mr-1" />
                    ...
                    <div className="relative" ref={openActionMenuId === record.id ? menuRef : null}>
                      <button 
                        onClick={(e) => { e.stopPropagation(); setOpenActionMenuId(openActionMenuId === record.id ? null : record.id); }} 
                  ...
                  <div className="text-sm text-slate-600 mb-2">{record.supplier}</div>
