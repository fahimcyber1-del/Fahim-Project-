...
import { SOPRecord, SOPStatus, SOPDepartment } from './types';
import { FileText, CheckCircle, Clock, AlertTriangle, BarChart2 } from 'lucide-react';
...
export function SopDashboard({ records }: SopDashboardProps) {
  const statusCounts = records.reduce((acc, r) => {
    acc[r.status] = (acc[r.status] || 0) + 1;
    return acc;
  }, {} as Record<SOPStatus, number>);

  const departmentCounts = records.reduce((acc, r) => {
    acc[r.department] = (acc[r.department] || 0) + 1;
    return acc;
  }, {} as Record<SOPDepartment, number>);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-slate-600 font-bold text-sm">Total SOPs</h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:p-6">
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
          <h3 className="text-lg font-black text-slate-900 mb-6 flex items-center gap-2">
            <BarChart2 className="w-5 h-5 text-indigo-600" /> SOPs by Department
          </h3>
             ...
             {[...records]
               ...
               <div key={r.id} className="border border-slate-200 rounded-lg p-3 hover:bg-slate-50 transition-colors">
                 ...
                 <div className="flex justify-between items-center text-xs text-slate-500 mt-2">
                    <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded font-bold uppercase tracking-wider">{r.department}</span>
                    <span className="font-medium text-indigo-600">{r.status}</span>
                 </div>
               </div>
             ))}
             {records.length === 0 && (
                <p className="text-sm font-medium text-slate-500 text-center py-6">No recent SOPs found.</p>
             )}
           </div>
        </div>
      </div>
