...
export interface ProductionQualityExportOptions {
...
interface ExportModalProps {
  onClose: () => void;
  onExportPDF: (options: ProductionQualityExportOptions) => void;
  onExportCSV: (options: ProductionQualityExportOptions) => void;
}

export function ExportModal({ onClose, onExportPDF, onExportCSV }: ExportModalProps) {
  const [options, setOptions] = useState<ProductionQualityExportOptions>({
    includeDefects: true,
    includeSignature: true,
  });

  const handleToggle = (key: keyof ProductionQualityExportOptions) => {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          <label className="flex items-center gap-3 cursor-pointer">
            <input 
              type="checkbox" 
              checked={options.includeDefects} 
              onChange={() => handleToggle('includeDefects')}
              className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
            />
            <span className="text-sm font-medium text-slate-700">Detailed Defects</span>
          </label>
          <label className="flex items-center gap-3 cursor-pointer">
            <input 
              type="checkbox" 
