...
export function RiskAssessmentList({ records, onView, onEdit, onDelete, onNew }: RiskAssessmentListProps) {
  ...
  const handleExportPDF = (dataToExport: RiskRecord[] = filteredRecords) => {
    const doc = new jsPDF();
  ...
  const handleExportExcel = (dataToExport: RiskRecord[] = filteredRecords) => {
    ...
    XLSX.utils.book_append_sheet(wb, worksheet, 'Risks');
    XLSX.writeFile(wb, 'risks_report.xlsx');
  };

  const handleDeleteSelected = () => {
    setConfirmDelete({ ids: Array.from(selectedIds), message: `Are you sure you want to delete ${selectedIds.size} risk assessment(s)?` });
  };
  ...
  return (
    <div className="border border-slate-200 bg-white rounded-lg shadow-sm">
      <div className="border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-lg font-bold text-slate-900">Risk Assessment List</h2>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-slate-600 text-center">Level</th>
                            ...
                            {openActionMenuId === record.id && (
                              <div className="absolute right-0 mt-1 w-40 bg-white border border-slate-200 rounded-md shadow-lg z-20 flex flex-col p-1 text-sm">
                                <button onClick={() => { setOpenActionMenuId(null); onEdit(record.id); }} className="px-3 py-2 text-slate-700 hover:bg-slate-50 flex items-center gap-2 rounded">
                                 ...
                                 <button onClick={() => { setOpenActionMenuId(null); setConfirmDelete({ ids: [record.id], message: 'Are you sure you want to delete this risk assessment?' }); }} className="px-3 py-2 text-rose-600 hover:bg-rose-50 flex items-center gap-2 rounded">
                ...
                <div key={record.id} className="group bg-white border border-slate-200 hover:border-blue-300 rounded-xl p-5 shadow-sm hover:shadow transition-all flex flex-col relative">
                    ...
                    <div className="absolute top-2 right-2 z-10">
                       ...
                       {openActionMenuId === record.id && (
                         <div className="absolute right-0 mt-1 w-40 bg-white border border-slate-200 rounded-md shadow-lg z-20 flex flex-col p-1 text-sm">
                           ...
                           <button onClick={() => { setConfirmDelete({ ids: [record.id], message: 'Are you sure you want to delete this risk assessment?' }); setOpenActionMenuId(null); }} className="px-3 py-2 text-rose-600 hover:bg-rose-50 flex items-center gap-2 rounded">
                              <Trash2 className="w-4 h-4" /> Delete
                           </button>
                         </div>
                       )}
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-100 grid grid-cols-2 gap-4 cursor-pointer" onClick={() => onView(record.id)}>
                    ...
                    <div>
                      ...
                      <span className={`px-2 py-0.5 rounded-md text-xs font-bold ${getStatusColor(record.status)}`}>
                        {record.status}
                      </span>
                    </div>
                  </div>
