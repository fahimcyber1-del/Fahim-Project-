...
import { Search, Filter, FileDown, PlusCircle, ArrowUpRight, ArrowDownRight, Minus, Eye, MoreHorizontal, ChevronDown, ChevronUp, FileText, Download, Edit, Trash2 } from 'lucide-react';
...
export function KpiList({ records, onView, onEdit, onDelete, onNew }: KpiListProps) {
  ...
  const getPerformanceIcon = (current: number, target: number) => {
    // Basic logic assuming higher is better for some, lower for others. 
    // Here we'll just do a generic check or just show based on status.
    // We can simulate trend with random arrow for demo purposes, or based on history.
    return <Minus className="w-4 h-4 text-slate-400" />;
  };

  const handleExportPDF = (dataToExport: KpiRecord[] = filteredRecords) => {
    const doc = new jsPDF();
    doc.text('KPI Tracking Report', 14, 15);
    autoTable(doc, {
      head: [['ID', 'Title', 'Category', 'Dept', 'Target', 'Current', 'Status']],
      body: dataToExport.map(r => [
        r.id, r.title, r.category, r.department, `${r.target} ${r.unit}`, `${r.currentValue} ${r.unit}`, r.status
      ]),
      startY: 20,
    });
  ...
  const handleExportExcel = (dataToExport: KpiRecord[] = filteredRecords) => {
    const ws = XLSX.utils.json_to_sheet(dataToExport.map(r => ({
      ID: r.id, Title: r.title, Category: r.category, Department: r.department, 
      Target: `${r.target} ${r.unit}`, Current: `${r.currentValue} ${r.unit}`, Status: r.status, 
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col h-full">
      <div className="border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
        <h2 className="text-lg font-bold text-slate-900">KPI Records</h2>
        <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <PlusCircle className="w-4 h-4" /> New KPI
            </button>
          </div>
      </div>
      ...
      <div className="overflow-x-auto flex-1 h-full min-h-0 relative">
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-slate-50 border-b border-slate-200 top-0 sticky z-10">
            <tr>
              <th className="px-6 py-4 font-semibold text-slate-600">KPI</th>
              <th className="px-6 py-4 font-semibold text-slate-600">Target</th>
              <th className="px-6 py-4 font-semibold text-slate-600">Current</th>
              <th className="px-6 py-4 font-semibold text-slate-600">Status</th>
              <th className="px-6 py-4 font-semibold text-slate-600">Owner</th>
               ...
               <tr key={record.id} className="hover:bg-slate-50 transition-colors group cursor-pointer" onClick={() => onView(record.id)}>
                 <td className="px-6 py-4">
                   <div className="flex flex-col">
                     ...
                     <span className="text-xs font-medium text-slate-500">{record.id} • {record.department}</span>
                   </div>
                 </td>
                 <td className="px-6 py-4 font-semibold text-slate-700">{record.target} <span className="text-xs text-slate-500 font-medium">{record.unit}</span></td>
                 <td className="px-6 py-4">
                   <div className="flex items-center gap-2">
                     <span className="font-black text-slate-900">{record.currentValue} <span className="text-xs text-slate-500">{record.unit}</span></span>
                   </div>
                 </td>
                 <td className="px-6 py-4">
                 ...
                 <td className="px-6 py-4 font-medium text-slate-600 text-sm">{record.owner}</td>
