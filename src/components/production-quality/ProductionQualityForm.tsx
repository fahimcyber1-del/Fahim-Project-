...
export function ProductionQualityForm({ initialData, onSave, onCancel, sections, lines, units, shifts }: FormProps) {
  const [formData, setFormData] = useState<Partial<QualityRecord>>({
    id: `PQ-${format(new Date(), 'yyyy')}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
  ...
  const handleDefectChange = (index: number, field: 'type' | 'count', value: string | number) => {
    const updatedDefects = [...(formData.topDefects || [])];
    updatedDefects[index] = { ...updatedDefects[index], [field]: value };
    
    const totalDefects = updatedDefects.reduce((sum, defect) => sum + (Number(defect.count) || 0), 0);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (['inspectedQuantity', 'defectedQuantity', 'reworkedQuantity', 'rejectedQuantity'].includes(name)) {
  ...
  const uniqueStyles = Array.from(new Set(INITIAL_ORDERS.map(o => o.styleNumber))).filter(Boolean);
  const uniqueBuyers = Array.from(new Set(INITIAL_ORDERS.map(o => o.buyerName))).filter(Boolean);
  const uniqueDefects = Array.from(new Set(INITIAL_DEFECTS.map(d => d.name))).filter(Boolean);
  ...
  return (
    <div className="space-y-4 max-w-4xl mx-auto">
      ...
      <datalist id="buyers-list">
        {uniqueBuyers.map(b => <option key={b} value={b} />)}
      </datalist>
      <datalist id="defects-list">
        {uniqueDefects.map(d => <option key={d} value={d} />)}
      </datalist>
          ...
          <div className="p-4 sm:p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Production Line</label>
                    <select 
                      name="line" 
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-bold uppercase tracking-wider text-slate-900 border-b pb-2 flex justify-between items-center">
                  Inspection Data
                  <button type="button" onClick={calculatePassed} className="text-[10px] text-blue-600 font-bold px-2 py-1 bg-blue-50 rounded hover:bg-blue-100">Auto-Calculate Passed</button>
                </h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Passed Qty</label>
                    <input 
                      type="number" 
                      name="passedQuantity" 
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[10px] font-semibold text-slate-600 mb-1">Defected Qty</label>
                    <input 
                      type="number" 
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-slate-600 mb-1">Reworked Qty</label>
                    <input 
                      type="number" 
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-slate-600 mb-1">Rejected Qty</label>
                    <input 
                      type="number" 
                ...
                <div className="space-y-4 border border-slate-200 p-4 rounded-lg bg-slate-50">
                  <div className="flex justify-between items-center">
                    <label className="block text-sm font-bold text-slate-900">Defects List</label>
                    <button type="button" onClick={addDefectField} className="text-xs text-blue-600 font-bold px-3 py-1.5 bg-blue-100 rounded hover:bg-blue-200 transition-colors shadow-sm">+ Add Defect</button>
                  </div>
                  ...
                  {formData.topDefects?.map((defect, index) => (
                    <div key={index} className="flex gap-2 items-center">
                      ...
                      <input 
                        type="number" 
                        placeholder="Qty"
                        value={defect.count === 0 && defect.type === '' ? '' : defect.count}
                        onChange={(e) => handleDefectChange(index, 'count', e.target.value === '' ? '' : parseInt(e.target.value, 10))}
                        min="0"
                  ...
                  <div className="pt-3 mt-2 flex justify-between items-center border-t border-slate-200">
                    <span className="text-sm font-bold text-slate-600">Total Defects From List:</span>
                    <span className="text-lg font-black text-rose-600">{formData.topDefects?.reduce((sum, d) => sum + (Number(d.count) || 0), 0) || 0}</span>
                  </div>
                </div>

                {(Number(formData.passedQuantity) + Number(formData.defectedQuantity) + Number(formData.reworkedQuantity) + Number(formData.rejectedQuantity) !== Number(formData.inspectedQuantity)) && (
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded text-xs text-amber-700 flex items-start gap-2">
                    <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
                    Warning: The sum of passed, defected, reworked, and rejected quantities does not equal the total inspected quantity.
                  </div>
                )}
