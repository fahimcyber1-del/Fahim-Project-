...
export function ProductionQualityModule() {
  const [records, setRecords] = useApiStorage(
    "aqm_productionquality_records",
    INITIAL_RECORDS,
  );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "app-fullscreen",
      handleFullscreen as EventListener,
    );
    return () =>
  ...
  return (
    <div
      className={`w-full h-full flex flex-col ${isFullscreen ? "bg-slate-50" : ""}`}
    >
