...
import { ArrowLeft, Edit3, FileText, User, Calendar, Tag, Paperclip, Download, History } from 'lucide-react';
...
export function DocumentDetail({ record, onBack, onEdit }: DocumentDetailProps) {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Obsolete': return 'bg-rose-100 text-rose-800 border-rose-200';
    ...
    <div className="max-w-5xl mx-auto w-full pb-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={onEdit} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-sm font-bold text-sm transition-colors">
          <Edit3 className="w-4 h-4" /> Edit Document
        </button>
      </div>
         ...
         <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
               <div className="border-b border-slate-100 px-6 py-4 flex items-center justify-between bg-slate-50">
                 <div className="flex items-center gap-2">
                   ...
                   <h3 className="font-bold text-slate-800 uppercase tracking-widest text-sm">Description</h3>
                 </div>
               </div>
               <div className="p-4 sm:p-6">
                  {record.description ? (
                    ...
                    <p className="text-sm text-slate-500 italic text-center py-4">No description provided.</p>
                  )}
               </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:p-6">
                 ...
                 <div>
                   <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Publisher</p>
                   {record.publisher ? (
                     <div className="flex items-center gap-2">
                       <User className="w-4 h-4 text-slate-400" />
                       <p className="text-sm font-bold text-slate-900">{record.publisher}</p>
                     </div>
                   ) : <p className="text-sm text-slate-400 italic">Not set</p>}
                 </div>
               </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
               ...
               <div className="space-y-4">
                 <div className="flex items-start gap-3">
                   ...
                   <div>
                     <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Category & Dept</p>
                     <p className="text-sm font-bold text-slate-900">{record.category}</p>
                     {record.department && <p className="text-xs text-slate-600">{record.department}</p>}
                   </div>
                 </div>
