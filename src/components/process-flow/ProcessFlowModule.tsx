...
import { Workflow, List, PlusCircle, LayoutDashboard } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between mb-6 flex-shrink-0">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Process Flowcharts</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Design and manage operational workflows</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 border-b border-slate-200 pb-4 flex-shrink-0 overflow-x-auto">
        ...
        >
          <List className="w-4 h-4" /> All Flows
        </button>
        <button
      ...
      <div className="flex-1 overflow-hidden flex flex-col min-h-0">
        {viewState.type === 'dashboard' && <ProcessFlowDashboard records={records} />}
        {viewState.type === 'list' && (
          <ProcessFlowList 
            records={records}
