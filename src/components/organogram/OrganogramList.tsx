...
import { Search, Filter, AlertTriangle, Eye, Edit3, Trash2, PlusCircle, Users } from 'lucide-react';

interface OrganogramListProps {
...
export function OrganogramList({ records, onView, onEdit, onDelete, onNew }: OrganogramListProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden min-h-0">
      <div className="p-3 px-4 border-b border-slate-200 bg-slate-50 flex flex-row items-center justify-between gap-3 flex-shrink-0">
        ...
        >
          <PlusCircle className="w-4 h-4" /> <span className="hidden sm:inline">New Chart</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4 min-h-0">
        {filteredRecords.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 text-slate-500 h-full border-2 border-dashed border-slate-200 rounded-xl">
            ...
            <p className="font-medium text-slate-900">No charts found</p>
              ...
              <div key={record.id} className="group bg-white border border-slate-200 hover:border-indigo-300 rounded-xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col">
                ...
                <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                  <div>
                    ...
                    <span className="ml-2 px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-100 text-slate-600 uppercase tracking-wider">
                       {record.department}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 text-xs font-bold text-slate-500">
                     <Users className="w-3.5 h-3.5" /> {record.nodes.length} Roles
                  </div>
                </div>
              </div>
            ))}
