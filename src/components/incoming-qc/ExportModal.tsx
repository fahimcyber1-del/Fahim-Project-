...
export interface IncomingQCExportOptions {
  includeGeneralDetails: boolean;
  includeSpecificDetails: boolean; // Fabric or Accessories details
  includeNotes: boolean;
  includeAttachments: boolean; // PDF only
}

interface ExportModalProps {
  onClose: () => void;
  onExportPDF: (options: IncomingQCExportOptions) => void;
  onExportCSV: (options: IncomingQCExportOptions) => void;
}
...
export function ExportModal({
  ...
  const [options, setOptions] = useState<IncomingQCExportOptions>({
    includeGeneralDetails: true,
    includeSpecificDetails: true,
    includeNotes: true,
    includeAttachments: false,
  });

  const handleToggle = (key: keyof IncomingQCExportOptions) => {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={options.includeGeneralDetails}
              onChange={() => handleToggle("includeGeneralDetails")}
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={options.includeSpecificDetails}
              onChange={() => handleToggle("includeSpecificDetails")}
              className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
            />
            <span className="text-sm font-medium text-slate-700">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              onChange={() => handleToggle("includeAttachments")}
              className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
            />
            <span className="text-sm font-medium text-slate-700">
