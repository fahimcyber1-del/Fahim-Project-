...
import { ProcessFlowRecord, FlowStatus } from './types';
import { Workflow, CheckCircle, Clock, AlertTriangle, BarChart2 } from 'lucide-react';
...
export function ProcessFlowDashboard({ records }: ProcessFlowDashboardProps) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-slate-600 font-bold text-sm">Total Flows</h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:p-6">
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
          <h3 className="text-lg font-black text-slate-900 mb-6 flex items-center gap-2">
            <BarChart2 className="w-5 h-5 text-indigo-600" /> Flows by Department
          </h3>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
           ...
           <div className="space-y-3">
             ...
             {records.length === 0 && (
                <p className="text-sm font-medium text-slate-500 text-center py-6">No recent flows found.</p>
             )}
           </div>
        </div>
      </div>
